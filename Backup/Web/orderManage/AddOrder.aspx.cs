...
public partial class orderManage_AddOrder : PageBase
{
    ...
    protected void Save_Click(object sender, ImageClickEventArgs e)
    {
            ...
            try
            {
                ...
                orderMod.orderNum = txtorderNum.Text.Trim();//订单编号
                ...
                orderMod.发货状态 = rbl发货状态.SelectedValue;
                //添加订单详细
                    ...
                    if (flag > 0)
                    {
                        ...
                        if (flag2)
                        {
                            ...
                            else
                            {
                                ...
                                ScriptManager.RegisterStartupScript(Page, typeof(Page), "BGYPSQLB", "DetailsPageControl.CloseBox();alert('您没有要添加的型号，添加失败！')", true);
                            }
                        }
                        else
                        {
                            ProOrdersDetailBLL.Delete(txtorderNum.Text.Trim());
                            ProOrdersBLL.Delete(txtorderNum.Text.Trim());
                            ScriptManager.RegisterStartupScript(Page, typeof(Page), "BGYPSQLB", "DetailsPageControl.CloseBox();alert('添加失败！')", true);
                        }
                    }
                    else if(flag==-1)
                    {  //已经存在订单不能重复添加
                        ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "BGYPSQLB", "DetailsPageControl.CloseBox();", true);
                    }
                    else
                    {
                        ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "BGYPSQLB", "DetailsPageControl.CloseBox();alert('添加失败！')", true);
                    }
                }
            }
            catch (Exception ex)
