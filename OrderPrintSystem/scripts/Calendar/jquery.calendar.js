...
(function($){
		...
		draw : {
			...
			month : {
					...
					for( var i=0; i<data.settings.daystodraw; i++ ){
						...
						clonedDate = data.elements.dayblock.clone(true)
							.attr({
								'date'	: clonedDateFormat,
								'day'	: clonedDateObject.getDay()//,
                               // 'onclick':'addCalenderBySelectDay("'+clonedDateFormat+'");'
							})
							.css({
							...
							.find('p')
								.text( clonedDateObject.getDate() ).attr("onclick",'addCalenderBySelectDay("'+clonedDateFormat+'");')
							.end();
                            clonedDate.find('span').attr("id",clonedDateFormat+"-event");
						//liuxd 添加
				...
				{   // Cache the dates (make sure for internal purposes it doesn't extend after the last displaying date.
					...
					var daysInEvent = _private.event.calculateElementCount.apply(this, [values]),$event, $events;
					...
					if( !( 'elems' in values ) || values.elems.length < 1 ){
						...
						$event.find('p.title').text( '●' + (  ( values.begins.format(data.settings.maskeventlabel) + 
							(data.settings.maskeventlabelend !== '' ? data.settings.maskeventlabeldelimiter + values.ends.format( data.settings.maskeventlabelend ) : ''
							)+""+values.title
						) ) );
                       ...
                       // $event.find('p.title').attr("style","background:"+values.colors.original+";");
                       /**添加选择事件**/
                       // $event.attr('onclick',"pageindex('"+values.uid+"')");
                        $event.attr('data-id',values.uid);
                      
                       //$event.find('p.title').addClass("{background:"+values.colors.original+"}");
						// Start the events collection small with this element.
						$events = $event;
                 ...
                 //判断一天有多少事件
                  if(typeof(DayCount[values.begins.format('Y-m-d')])=='undefined'){
                 
                     DayCount[values.begins.format('Y-m-d')]=1;
                  }else {
                 ...
                 //添加全天共多少事件
                     $("#"+values.begins.format('Y-m-d')+"-event").attr('onclick','window.location.href=\'../Calendar/Day.aspx?courrentday='+values.begins.format('Y-m-d')+'\'').text(DayCount[values.begins.format('Y-m-d')]+"个事件");
				...
				position : function( speed, ease, detect ){
					...
					// Make sure we've got values.
					if( data && values ){
						var inDayIndex=DayCount[values.begins.format('Y-m-d')];
						...
						$events.each(function( i, event ){
                            ...
                            var newStylesMain = {
								top				: data.cache.dayHeight * Math.floor((diffDays-1)/7) + 2 + 28,//+((inDayIndex)*5),
								left			: ( data.cache.dayWidth * ((diffDays-1)%7) ) + ( i > 0 ? 0 : 2 ),//-((1-inDayIndex)*15),
								width			: data.cache.dayWidth - ( spanning && i==0 ? 2 : 4 ),//+((1-inDayIndex)*15),
								height			: data.cache.incrementHeight,
							...
							$event.attr('title',values.title||'');
							/**如果一天有多个事件 则进行tab设置**/
                           
                            //添加tab页面
                          
                            if($event.find(".pageindex").find(".tab").length==0){
                            var _left=5;
                            
                            if(inDayIndex==1){
                                _left=5;
                            }else{
                                for(var pos=1;pos<inDayIndex;pos++){
                                    _left=36*(pos);
                                }
                                _left+=5;
                            }
                            
                            $event.find(".pageindex").append("<span class='tab' pagedata-id='"+values.uid+"' style='z-index:"+_left+";background:"+values.colors.original+";margin:-1px 0px 0px "+(_left)+"px;' onclick='pageindex(\""+values.uid+"\")'><center>&nbsp;<center></span>");
                           }
   ...
   var DayCount=new Array();
	var fragments = {
        ...
        //ondblclick="CalendarByUid(this)"
		event : $([
			'<div class="ui-'+plugin_name+'-event mid"  onclick="CalendarByUserDetailMyBox();">',
				'<p class="resize-top" />',
                '<p class="title" />',
                '<p class="pageindex" style="display:none;" />',
			...
			.bind('mousedown.'+plugin_name,_private.drag.start)//滑动效果 这里需要隐藏
				...
				if ( ! data ) {
					...
					switch( data.type ){
						...
						case const_month : // Pass the data to save a call to the data API.
							...
							data.elements.dayblock.append('<p/><span/>');
					...
					switch( key ){
						...
						case 'startdate' :
							...
							var $dates		= $('div.ui-'+plugin_name+'-date',data.elements.container).removeClass('ui-'+plugin_name+'-today'),
								todayDate	= $[plugin_name].date().format('Y-m-d');
                           //debugger;
							//$('div.ui-'+plugin_name+'-date p').after("<span>aaaa</span>")
							switch( data.type ) {
...
//								});
	...
	$[plugin_name].lang = {
		...
		'en' : {
			...
			// Text date/time representations.
			short_month 	: Array('一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'),
			long_month		: Array('一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'),
			short_day		: Array('星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'),
			long_day		: Array('星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'),
			// Text labels.
			label_remove	: '删除',
			label_today		: '今日'
		}
