...
function ZxjayTreeNode(userid, deptid,unitid) {
    var self = this;
    user = userid;
    dept = deptid;
    unit = unitid;
    ...
    this.childArea = null;
    this.parent = null;
    this.path = "~/userFile/" + unitid + "/" + deptid + "/" + userid + "/";
    defaultFilePath = "~/userFile/" + unitid + "/" + deptid + "/" + userid + "/";
    this.Show = function () {
    ...
    this.CreateChildren = function () {
        if (this.childrenNodes == null) {
            ...
            for (var i = 0; i < this.folders.length; i++) {
                var node = new ZxjayTreeNode(user, dept,unit);
        ...
        //更新当前路径
        $$(pathID).innerHTML = self.path.replace(defaultFilePath, "我的资料库/");
