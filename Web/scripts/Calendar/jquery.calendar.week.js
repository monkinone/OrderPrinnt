...
(function($){
			...
			week : {
				...
				cal : function( data ){
					...
					data.elements.dayblock.css({
						width	: data.cache.resourceWidth-15,
					...
					for( var i=0; i<data.settings.daystodisplay; i++ ){
						...
						for( var r=0; r<data.cache.resourcecount; r++ ){
							...
							clonedDate = data.elements.dayblock.clone(true)
								.css( 'left', ( (data.cache.dayWidth-15) * i ) + ( (data.cache.resourceWidth-15) * r ) )
								.attr({
						...
						clonedDateLabel = data.elements.datelabel.clone(true)
							.css( 'left', (data.cache.dayWidth-15) * i )
							.attr({
								'date'	: clonedDateFormat,
								'day'	: clonedDateObject.getDay(),
							})
					...
					if( data && values ){
						...
						$events.each(function( i, event ){
							...
							var newStylesMain = {
								...
								left			: (data.cache.dayWidth-15) * ( data.settings.startdate.getDaysBetween( values.cache.begins, true ) + i ) + ( (data.cache.resourceWidth-15) * values.resource ),
								width			: ( values.resource !== null ? data.cache.resourceWidth-15 : data.cache.dayWidth-15 ) - 1,
	...
	var fragments = {
		...
		event : $([
			'<div class="ui-'+plugin_name+'-event mid" onclick="CalendarByUid(this)">',
				'<p class="resize-top" />',
				'<p class="title" />',
